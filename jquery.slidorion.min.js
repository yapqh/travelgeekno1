...
*/
(function($){$.fn.extend({slidorion:function(options)
{var defaults={autoPlay:true,easing:"",
effect:"fade",first:1,interval:5E3,hoverPause:false,speed:1E3};
...
sliderCount=$("#slider > div",obj).size();obj.data("slideCount",sliderCount);
accordionCount=$("#accordion > .link-header",obj).size();
obj.data("accordCount",accordionCount);if(sliderCount==accordionCount){
  if(autoPlay==true){
    var autoPlaying=setInterval(function(){playSlider(current,effect,speed,easingOption)},interval);
    obj.data("interval",autoPlaying)
  }
    if(hoverPause==true&&autoPlay==true)obj.hover(function(){
      intervalPause=true;stopAuto()},function(){intervalPause=false;restartAuto()});
      resetLayers();$("#slider > div:eq("+(current-1)+")",obj).css("z-index",zPos);
      zPos++;
      if(effect!="fade"||effect!="none"){$("#slider > div",obj).css({"top":"0","left":"-600px"});
      $("#slider > div:eq("+(current-1)+")",obj).css({"top":"0","left":"0"})}$(".link-content",obj).hide();
      $("#accordion .link-header:eq("+(current-1)+")",obj).addClass("active").next().show();
      if(controlNav){obj.append('<div class="'+controlNavClass+" "+controlNavClass+'-left"></div><div class="'+controlNavClass+
